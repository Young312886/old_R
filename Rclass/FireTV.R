...
#url <- "https://www.amazon.com/product-reviews/B0791TX5P5/ref=cm_cr_arp_d_viewopt_srt?ie=UTF8&filterByStar=all_stars&reviewerType=all_reviews&sortBy=recent&pageNumber="
#url <- " your url "
...
# 5. Save the output
#write.csv(data.frame(A),"FireTV.csv") #you can change the file name
...
#
data <- read.csv("FireTV.csv") #change the name of the file
data<-data[,2]
head(data,10)
tail(data,10)
...
docs <- tm_map(docs, removeWords, c( "work", "stick", "fire", "use", "amazon", "just", "product", "firestick", "devic", "app", "will", "month", "now", "even", "year", "¡®s",  "anoth", "still", "everi", "two", "everyth", "also", "noth")) 
...
write.csv(unlist(dtm), "FireTV_N_DTM.csv")

class(dtm)

tdm <- TermDocumentMatrix(docs)   

write.csv(as.matrix(dtm),"FireTV_Term_Document_Matrix.csv")

write.csv(mdtm,"FireTV_Document_Term_Matrix.csv")
...
m <- as.matrix(dtm)   
dim(m)   
write.csv(m, file="FireTV_DocumentTermMatrix.csv")   
...
head(freq, 100)   #This will show you the top 20 most frequently mentioned words
tail(freq, 50)   #This will show you the bottom 10 least frequently mentioned words
...
dtmss <- removeSparseTerms(dtm, 0.90) # This makes a matrix that is only 15% empty space.
dtmss
...
op = par(mfrow = c(2, 1))
plot(cut(hcd, h = 30)$upper, main = "Upper tree of cut at h=100")
plot(cut(hcd, h = 30)$lower[[2]], main = "Second branch of lower tree with cut at h=100")


### 5.2 K-means clustering   #iteration, stopping rule
library(fpc)   
library(cluster)  
dtms <- removeSparseTerms(dtm, 0.95) # Prepare the data (max 15% empty space)   
